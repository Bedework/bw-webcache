...
public class FeedModel {
  public static URI getUri(final ParsedRequest preq,
    ...
    return switch (preq.getAction()) {
      case "icsDays" -> buildIcsDays(preq, config);
      case "jsonDays" -> buildJsonDays(preq, config);
      case "rssDays" -> buildRssDays(preq, config);
      default -> throw new RuntimeException(
              "Unsupported action: " + preq.getAction());
    };
  }

  public static String getContentType(final ParsedRequest preq) {
    return switch (preq.getAction()) {
      case "icsDays" -> "text/calendar; charset=UTF-8";
      case "jsonDays" -> "application/javascript; charset=UTF-8";
      case "rssDays" -> "application/rss+xml; charset=UTF-8";
      default -> throw new RuntimeException(
              "Unsupported action: " + preq.getAction());
    };
  }
